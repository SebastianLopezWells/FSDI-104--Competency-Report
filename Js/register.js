...
//constructor
let c=0;
function Pet(name, age, gender,breed, service, ownerName, contactPhone, Illness){
...
function clearInputs(){
    ...
    document.getElementById("txtPetIllness").value="";
}
...
function register(){
    ...
    let petIllness=document.getElementById("txtPetIllness").value;
    //create a obj
    let newPet = new Pet(petName, petAge, petGender, petBreed, petService, petOwnerName, petContact,petIllness);
    if(isValid(newPet))
    {
        //push obj
...
function search(){    
    ...
    for (let i = 0; i < petSalon.pets.length; i++) 
    {
        ...
        if(searchTo.toLowerCase() == pet.name.toLowerCase()){
            ...
            console.log("Searching"+ searchTo);
        }else{
...
window.onload = init; //Calls the init function by loading the document.